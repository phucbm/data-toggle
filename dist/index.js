/*!
 * @phucbm/data-toggle 0.0.1
 * https://github.com/phucbm/data-toggle#readme
 *
 * @license MIT
 */
...
var createDataToggle = (config = {}) => {
  ...
  const boundHandlers = /* @__PURE__ */ new WeakMap();
  const log = (...args) => {
    if (options.debug) {
  ...
  const findToggleTarget = (toggleElement) => {
  ...
  const bindEvents = () => {
    ...
    try {
      ...
      const clickHandler = options.debounceDelay > 0 ? import_lodash.default.debounce(handleToggleClick, options.debounceDelay) : handleToggleClick;
  ...
  return {
    ...
    toggle(className, elementSelector) {
      if (isDestroyed) {
      ...
      const toggleEvent = new CustomEvent("datatoggle", {
        detail: {
          ...
          trigger: null,
          // No trigger for programmatic toggles
          active: !wasActive
        }
      });
      document.dispatchEvent(toggleEvent);
    },
    isActive(className, elementSelector) {
      if (isDestroyed) {
